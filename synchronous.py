...
"""
    ...
    vesion:
        2019-11-15 15:05:21 v1.0.1 格式化化输出界面,更换数据库引擎,简化逻辑
        2019-11-19 11:03:47 v1.0.2 修改get_table函数,增加筛选
        2019-11-25 16:08:52 v1.0.3 修复bug,过滤空表,优化正则规则
...
def get_database_info(cursor, database):
    ...
    database_info['tables'] = [r['table_name'] for r in result]
...
def get_table_info(cursor, database, table):
    ...
    table_size_sql = f"""
        select table_name, concat(truncate(data_length,2),' bytes') as table_size
        from information_schema.tables
        where table_name = '{table}' and table_schema = "{database}"
        """
    cursor.execute(table_size_sql)
    table_size = cursor.fetchall()[0]['table_size']
    return columns_info, table_size
...
def create_target_table(cursor, database, table, columns_info):
    '''
    创建目标表
    '''
    columns = [c['Field'] + ' ' + c['Type'] for c in columns_info]
    columns = ",".join(columns)
    cursor.execute(f"CREATE TABLE IF NOT EXISTS {database}.{table} ({columns})")
    ...
    def main(self, **columns):
       ...
       if mode == 'replace':
        ...
        for table in self.table_set:
            table_size = get_table_info(source_cursor, source['database'], table)[1]
        ...
        for table in self.table_set:
            ...
            if len(data):
                data.to_sql(table, target_engine, if_exists=mode, chunksize=2000, index=0)
                e = time.time()
    ...
    for mission in list(CONFIG.keys()):
        ...
        if mission_config['common']['switch'] == 'on':
           ...
           # key = datetime.datetime.strftime(
               ...
               # )
		   # value = "'" + f"{load_date}" + "'"
           # Synchronous(mission_config).main(key = value)
